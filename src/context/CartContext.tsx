...
interface CartContextType {
  items: CartItem[];
  addToCart: (item: CartItem, qty?: number) => void;  // ✅ qty support added
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  // ✅ CHANGED: qty parameter added (default 1 — purana code bhi chalega)
  const addToCart = (item: CartItem, qty: number = 1) => setItems(prev => {
  const e = prev.find(p => p.id === item.id);
  if (e) return prev.map(p => p.id === item.id ? { ...p, quantity: (p.quantity || 0) + qty } : p);
  ...
  const totalPrice = items.reduce((s, p) => s + (Number((p.price || "").replace(/[^\d]/g, "")) || 0) * (p.quantity || 0), 0)
