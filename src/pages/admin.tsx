...
export default function Admin() {
  ...
  const handleLogin = () => {
    if (pass === process.env.NEXT_PUBLIC_ADMIN_PASSWORD) { setLogged(true); setPass('') }
  ...
  const editProduct = (p: any) => { setEditId(p.id); setPForm({ title: p.title||'', price: p.price||'', description: p.description||'', category: p.category||'', condition: p.condition||'Good', image: p.image||'', image2: p.image2||'', videoUrl: p.videoUrl||'', featured: p.featured||false, inStock: p.inStock !== false }); window.scrollTo({ top: 0, behavior: 'smooth' }) }
  const editCategory = (c: any) => { setEditId(c.id); setCForm({ name: c.name||'', icon: c.icon||'⚙️', image: c.image||'', active: c.active !== false }); window.scrollTo({ top: 0, behavior: 'smooth' }) }

  const deleteItem = async (col: string, id: string) => { if (!confirm('Delete karein?')) return; try { await deleteDoc(doc(db, col, id)) } catch { alert('Error') } }
  ...
  if (!logged) return (
    ...
    <div className="min-h-screen bg-[#0A1929] flex items-center justify-center px-4">
      <div className="w-full max-w-sm bg-[#13293D] border border-[#1E3A52] rounded-2xl p-8 text-center">
        ...
        <input value={pass} onChange={e => setPass(e.target.value)} type="password" placeholder="Enter Password" onKeyDown={e => e.key === 'Enter' && handleLogin()} className="w-full px-4 py-3 bg-[#0D1F30] border border-[#1E3A52] rounded-xl text-sm text-white placeholder:text-gray-600 focus:outline-none focus:border-[#F5A623]/50 mb-4" autoFocus />
          ...
          <div className="bg-[#13293D] border border-[#1E3A52] rounded-2xl p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 text-xs text-gray-400 cursor-pointer"><input type="checkbox" checked={pForm.featured} onChange={e => setPForm({...pForm, featured: e.target.checked})} className="accent-[#F5A623]" /> ⭐ Featured</label>
              </div>

              <button onClick={saveProduct} className="w-full bg-[#F5A623] hover:bg-[#D4911E] text-[#0A1929] font-bold py-3 rounded-xl text-sm transition-all">{editId ? '💾 Update Product' : '➕ Add Product'}</button>
            </div>
          </div>
              ...
              {prods.map(p => (
                <div key={p.id} className="flex items-center gap-3 bg-[#0D1F30] border border-[#1E3A52] rounded-xl p-3 hover:border-[#F5A623]/20 transition-all">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${p.inStock !== false ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>{p.inStock !== false ? 'In Stock' : 'Out of Stock'}</span>
                  </div>
                  <div className="flex gap-1 flex-none">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-[#13293D] border border-[#1E3A52] rounded-2xl p-6">
            ...
            <div className="space-y-3">
              ...
              <button onClick={saveCategory} className="w-full bg-[#F5A623] hover:bg-[#D4911E] text-[#0A1929] font-bold py-3 rounded-xl text-sm transition-all">{editId ? '💾 Update Category' : '➕ Add Category'}</button>
            </div>
          </div>
