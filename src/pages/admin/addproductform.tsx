...
export default function AddProductForm({ onClose }: { onClose: () => void }) {
      ...
      try {
        ...
        const subs = await writeClient.fetch(
          `*[_type == "subcategory"] | order(title asc){
            _id,
            title,
            "parentCatId": parentCategory->_id,
            "parent": parentCategory->title
          }`
        )
        console.log("Categories fetched:", cats.length, cats)
        console.log("Subcategories fetched:", subs.length, subs)
        setCategories(cats)
        setSubcats(subs)
      } catch (error) {
  ...
  const filteredSubcats = catId
    ? subcats.filter((sub) => sub.parentCatId === catId)
    : []
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(error)
      alert("Product add nahi hua. Console check karo.")
    } finally {
      setSaving(false)
    }
  }
