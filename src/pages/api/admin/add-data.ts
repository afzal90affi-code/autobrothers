...
const serverClient = createClient({
  ...
  useCdn: false,
  token: process.env.SANITY_API_TOKEN, // Secret Token (Only Server)
});
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { type, title, slug, tag, parentId, price, condition, featured, newArrival, imageData, fileName } = req.body;

    let imageAsset = null;

    // 1. Image Upload to Sanity (Base64 to Buffer)
