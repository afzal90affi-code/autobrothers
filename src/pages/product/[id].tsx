...
import { ShoppingCart, MessageCircle, ArrowLeft, Truck, ShieldCheck, Wrench, Phone, Menu, X, Search } from "lucide-react"
    ...
    <>
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "name": product.title,
              "image": images,
              "description": product.description || "Quality used auto part",
              "sku": product._id,
              "brand": { "@type": "Brand", "name": "AutoBrothers" },
              "offers": {
                "@type": "Offer",
                "url": currentUrl,
                "priceCurrency": "PKR",
                "price": cleanPrice,
                "itemCondition": `https://schema.org/${product.condition === "Good" ? "UsedCondition" : "DamagedCondition"}`,
                "availability": product.inStock !== false ? "https://schema.org/InStock" : "https://schema.org/OutOfStock",
                "seller": { "@type": "Organization", "name": "AutoBrothers" },
              },
            }),
          }}
        />
              ...
              <>
                    ...
                    <div key={item.id} className="flex gap-3 bg-[#13293D] border border-[#1E3A52] rounded-xl p-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <button onClick={() => updateQty(item.id, item.quantity + 1)} className="w-6 h-6 rounded bg-[#1E3A52] flex items-center justify-center text-xs text-gray-400">+</button>
                          <button onClick={() => removeFromCart(item.id)} className="ml-auto text-red-400 text-xs">Delete</button>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="sticky bottom-0 bg-[#0A1929] border-t border-[#1E3A52] p-4 space-y-3">
                  ...
                  <a
                    ...
                    target="_blank"
                    className="w-full flex items-center justify-center gap-2 bg-[#25D366] text-white font-bold py-3 rounded-xl text-sm"
                  >
      ...
      <div className="min-h-screen bg-[#0A1929] text-white pt-24 pb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="mb-6">
            <Link href="/#products" className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-[#F5A623] mb-3 transition-colors">
            ...
            <div className="text-xs text-gray-500 flex items-center gap-2 flex-wrap">
              ...
              {product.catTitle && (
                <>
                  <span className="hover:text-gray-300">{product.catTitle}</span> <span>/</span>
                </>
              )}
              {product.subCatTitle && (
                <>
                  <span className="hover:text-gray-300">{product.subCatTitle}</span> <span>/</span>
                </>
              )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            ...
            <div>
              {/* Main Image */}
              <div className="relative aspect-square bg-[#0D1F30] border border-[#1E3A52] rounded-2xl overflow-hidden mb-4 group">
                <img
                ...
                {/* Image Counter */}
                {images.length > 1 && (
                  <div className="absolute top-4 left-4 bg-[#0A1929]/80 backdrop-blur-sm text-white text-xs font-bold px-3 py-1 rounded-md border border-[#1E3A52]">
                ...
                {images.length > 1 && (
                  <>
                    <button
                      ...
                      className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-[#0A1929]/80 backdrop-blur-sm hover:bg-[#F5A623] hover:text-[#0A1929] text-white flex items-center justify-center transition-all opacity-0 group-hover:opacity-100"
                    ...
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-[#0A1929]/80 backdrop-blur-sm hover:bg-[#F5A623] hover:text-[#0A1929] text-white flex items-center justify-center transition-all opacity-0 group-hover:opacity-100"
                ...
                <div className="flex gap-3 overflow-x-auto pb-2" style={{ scrollbarWidth: "thin" }}>
                  {images.map((img: string, idx: number) => (
                    <button
                      ...
                      className={`flex-none w-20 h-20 rounded-xl border-2 overflow-hidden transition-all ${
                        selectedImage === idx
                          ? "border-[#F5A623] ring-2 ring-[#F5A623]/30 scale-105"
                          : "border-[#1E3A52] hover:border-gray-500 opacity-70 hover:opacity-100"
                      }`}
                    >
            ...
            <div>
              ...
              <div className="mb-6">
                <span className="text-3xl font-black text-[#F5A623]">{product.price || "Call for Price"}</span>
              </div>

              {product.description && (
                <div className="mb-6 text-sm text-gray-400 leading-relaxed whitespace-pre-line border-l-2 border-[#F5A623]/30 pl-4">
                  {product.description}
                </div>
              )}
              ...
              <div className="space-y-3 mb-8">
                ...
                <a href={waLink(product.title, product.price, currentUrl)} target="_blank" className="w-full flex items-center justify-center gap-2 bg-[#25D366] hover:bg-green-600 text-white font-bold py-4 rounded-xl text-base transition-colors">
                  <MessageCircle size={20} /> Order via WhatsApp
                </a>
                <a href="tel:03222806245" className="w-full flex items-center justify-center gap-2 border border-[#1E3A52] bg-[#13293D] hover:bg-[#1A2B3C] text-gray-300 font-bold py-3 rounded-xl text-sm transition-colors">
                  <Phone size={16} /> Call: 0322-2806245
                </a>
              </div>

              <div className="grid grid-cols-3 gap-3">
                <div className="bg-[#13293D] border border-[#1E3A52] rounded-xl p-3 text-center">
                  <Truck size={20} className="mx-auto text-[#F5A623] mb-1" />
                  <p className="text-[10px] text-gray-400">All Pakistan Delivery</p>
                </div>
                <div className="bg-[#13293D] border border-[#1E3A52] rounded-xl p-3 text-center">
                  <ShieldCheck size={20} className="mx-auto text-[#F5A623] mb-1" />
                  <p className="text-[10px] text-gray-400">100% Original</p>
                </div>
                <div className="bg-[#13293D] border border-[#1E3A52] rounded-xl p-3 text-center">
                  <Wrench size={20} className="mx-auto text-[#F5A623] mb-1" />
      ...
      {/* WhatsApp FAB */}
      <a href={waLink(product.title, product.price, currentUrl)} target="_blank" className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-[#25D366] rounded-full flex items-center justify-center text-white shadow-2xl hover:scale-110 transition-transform">
